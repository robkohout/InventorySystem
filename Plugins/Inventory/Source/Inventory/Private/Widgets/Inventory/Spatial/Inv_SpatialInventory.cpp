...
void UInv_SpatialInventory::EquippedGridSlotClicked(UInv_EquippedGridSlot* EquippedGridSlot, const FGameplayTag& EquipmentTypeTag)
{
	...
	// Create an Equipped Slotted Item and add it to the Equipped Grid Slot (call EquippedGridSlot->OnItem Equipped())
	...
	UInv_ItemDescription* EquippedDescription, UCanvasPanel* Canvas) const
{
	...
	FVector2D ItemDescriptionSize = Description->GetBoxSize();
	FVector2D EquippedItemDescriptionSize = EquippedDescription->GetBoxSize();
